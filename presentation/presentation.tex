...
\begin{frame}{Substitutions}
  ...
  \begin{equation*}
    \inferrule{ }{\cdot : \operatorname{Sub} \cdot \; \Delta} \quad
    \inferrule{\sigma : \operatorname{Sub} \Gamma \; \Delta \\ t : \Delta \vdash A}
              {\sigma, t : \operatorname{Sub} \; (\Gamma, A) \; \Delta} \quad
    \inferrule{\sigma : \operatorname{Sub} \Gamma \; \Delta}
              {\operatorname{lift}_\lock \sigma : \operatorname{Sub} \; (\Gamma, \lock) \; (\Delta, \lock)}
  \end{equation*}
\end{frame}
...
\begin{frame}[fragile]
  \begin{align*}
    &\textit{subst} \; &&(\sigma, x) \; &&(\operatorname{var} \operatorname{i0}) &&= x \\
    &\textit{subst} \; &&(\sigma, \_) \; &&(\operatorname{var} \; (\operatorname{is} v)) &&= \textit{subst} \; \sigma \; (\operatorname{var} v) \\
    & \cdots \\
    &\textit{subst} \; &&\sigma \; &&(\operatorname{box} t) &&= \operatorname{box} \; (\textit{subst} \; (\operatorname{lock} \sigma \; \tikznode{lhd1}{$\lhd_\lock$}) \; t) \\
    &\textit{subst} \; &&\sigma \; &&(\operatorname{unbox} t \; m) &&= \operatorname{unbox} \; (\textit{subst} \; \sigma' \; t) \; m' \\
    &\mathrlap{\quad \text{where } m', \sigma' = \textit{rewind} \; m \; \sigma}
  \end{align*}
  \tikz[remember picture,overlay]{
    \draw (lhd1.north) -- ++(.5cm,1.5cm) node[anchor=south] {$\lhd_\lock : \Gamma \lhd \Gamma, \lock$};
  }
\end{frame}
...
%%     \inferrule{x : \text{Ne} \; \Gamma \; (A \to B) \\ y : \text{Nf} \; \Gamma \; A}{x \; y : \text{Ne} \; \Gamma \; B} \\
